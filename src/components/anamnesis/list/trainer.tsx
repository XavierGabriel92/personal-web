...
import CreateAnamnesisDialog from "../dialog/create-anamnesis-dialog";
					...
					<div className="flex flex-col gap-4">
						{data.anamnesis
							...
							.map((item) => (
								<AnamnesisCard
									key={item.id}
									anamnesis={item as Anamnesis}
									onEdit={() => { navigate({ to: `/trainer/anamnesis/${item.id}`, params: { anamnesisId: item.id } }); }}
								/>
							))}
					</div>
				</>
...
function AnamnesisCard({
	...
	anamnesis: Anamnesis;
	onEdit: (anamnesisId: string) => void;
}) {
	return (
		<Card className="gap-0 cursor-pointer" onClick={() => onEdit(anamnesis.id)}>
			<CardHeader>
				...
				<CardAction className="flex items-center gap-2">
					...
					<AnamnesisActions anamnesis={anamnesis} onEdit={onEdit} />
				</CardAction>
			</CardHeader>
		...
		<AlertDialog open={open} onOpenChange={onOpenChange}>
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						Tem certeza que deseja deletar a anamnese "{anamnesis.name}"? Esta
